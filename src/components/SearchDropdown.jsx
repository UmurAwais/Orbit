          ...
          {suggestions.length > 0 ? (
            <div className="space-y-0.5">
                  {suggestions.map((item, idx) => {
                    ...
                    const isYouTube = safeItem.toLowerCase().includes('youtube');
                    const isGoogle = safeItem.toLowerCase().includes('google');
                ...
                <div className="space-y-6 pt-2">
                  <section>
                    <div className="px-6 py-2 mb-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                      ...
                      {(Array.isArray(history) && history.length > 0) ? 'Recently Visited' : 'Suggested Topics'}
                    </div>
                    
                    <div className="space-y-0.5">
                      {(Array.isArray(history) && history.length > 0 ? history : (Array.isArray(trendingSearches) ? trendingSearches : [])).map((item, idx) => {
                        const safeItem = String(item || "");
                        return (
                          <div 
                            key={idx}
